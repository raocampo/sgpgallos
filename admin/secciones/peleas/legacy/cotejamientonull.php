            ...
            <form class="contenido_tolerancia" action="peleaGenerada.php" enctype="multipart/form-data" method="post">
                <div class=" card d-flex row ">
                    ...
                    <div class="d-flex flex-row">
                        ...
                        $parejaCercana = null;
                        $diferenciaCercana = 0;
                        $parejaIntermedia = null;
                        $diferenciaIntermedia = 5;
                        $parejaLimite = null;
                        $diferenciaLimite = 10;
                        $gallosLibres = [];
                        
                        if ((!empty($toleranciaPeso) && $toleranciaPeso != 0) && (!empty($toleranciaAltura) && $toleranciaAltura != 0)) :
                            $gallos = $conexion->prepare("SELECT * FROM gallos WHERE familiasId <> 0 AND ID NOT IN (SELECT DISTINCT galloL FROM coteja WHERE torneoId =:torneoId) AND ID NOT IN(SELECT DISTINCT gallov FROM coteja WHERE torneoId =:torneoId) AND torneoId =:torneoId ORDER BY tamañoReal, pesoReal ");
                            ...
                            for ($index1 = 0; $index1 < $cantidadGallos; $index1++) {
                                ...
                                $galloPeso1 = round($gallo1['pesoReal'], 2);
                                $galloAltura1 = round($gallo1['tamañoReal'], 2);
                                ...
                                for ($index2 = $index1 + 1; $index2 < $cantidadGallos; $index2++) {
                                    ...
                                    $galloPeso2 = round($gallo2['pesoReal'], 2);
                                    $galloAltura2 = round($gallo2['tamañoReal'], 2);
                        
                                    $diferenciaPeso = abs($galloPeso1 - $galloPeso2);
                                    $diferenciaAltura = abs($galloAltura1 - $galloAltura2);
                        
                                    // Verificar si la pareja cumple con las tolerancias de peso y altura
                                    if ($diferenciaPeso <= $toleranciaPeso && $diferenciaAltura <= $toleranciaAltura) {
                                        $diferenciaTotal = $diferenciaPeso + $diferenciaAltura;
                        
                                        if ($diferenciaTotal < $diferenciaCercana) {
                                            $parejaCercana = [$gallo1, $gallo2];
                                            $diferenciaCercana = $diferenciaTotal;
                                        } elseif ($diferenciaTotal < $diferenciaIntermedia) {
                                            $parejaIntermedia = [$gallo1, $gallo2];
                                            $diferenciaIntermedia = $diferenciaTotal;
                                        }
                                    } elseif ($diferenciaPeso <= $toleranciaPeso * 2 && $diferenciaAltura <= $toleranciaAltura * 2) {
                                        $diferenciaTotal = $diferenciaPeso + $diferenciaAltura;
                        
                                        if ($diferenciaTotal < $diferenciaLimite) {
                                            $parejaLimite = [$gallo1, $gallo2];
                                            $diferenciaLimite = $diferenciaTotal;
                                        }
                                    }
                                }
                        
                                // Si no se encontró pareja, agregar el gallo a la lista de gallos libres
                        ...
                        // Verificar la pareja más cercana y agregarla a la lista de parejas
                        if ($parejaCercana) {
                            $parejas[] = $parejaCercana;
                            $ids_cotejados[] = $parejaCercana[0]['ID'];
                            $ids_cotejados[] = $parejaCercana[1]['ID'];
                        } elseif ($parejaIntermedia) {
                            $parejas[] = $parejaIntermedia;
                            $ids_cotejados[] = $parejaIntermedia[0]['ID'];
                            $ids_cotejados[] = $parejaIntermedia[1]['ID'];
                        } elseif ($parejaLimite) {
                            $parejas[] = $parejaLimite;
                            $ids_cotejados[] = $parejaLimite[0]['ID'];
                            $ids_cotejados[] = $parejaLimite[1]['ID'];
                        }
