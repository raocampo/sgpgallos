                            ...
                            if ((!empty($toleranciaPeso) && $toleranciaPeso != 0) || (!empty($toleranciaAltura) && $toleranciaAltura != 0)) :
                                ...
                                foreach ($gallos as $gallo1) {
                                    ...
                                    foreach ($gallos as $gallo2) {
                                        ...
                                        $diferenciaPeso = abs($gallo1['pesoReal'] - $gallo2['pesoReal']);
                                        $diferenciaAltura = abs($gallo1['tamañoReal'] - $gallo2['tamañoReal']);
                            
                                        if ((!empty($toleranciaPeso) && $toleranciaPeso != 0) && empty($toleranciaAltura)) {
                                            // Solo se especificó tolerancia de peso
                                            if ($diferenciaPeso <= $toleranciaPeso && $diferenciaPeso < $diferenciaOptima) {
                                                $parejaOptima = $gallo2;
                                                $diferenciaOptima = $diferenciaPeso;
                                            }
                                        } elseif (empty($toleranciaPeso) && (!empty($toleranciaAltura) && $toleranciaAltura != 0)) {
                                            // Solo se especificó tolerancia de altura
                                            if ($diferenciaAltura <= $toleranciaAltura && $diferenciaAltura < $diferenciaOptima) {
                                                $parejaOptima = $gallo2;
                                                $diferenciaOptima = $diferenciaAltura;
                                            }
                                        } elseif ((!empty($toleranciaPeso) && $toleranciaPeso != 0) && (!empty($toleranciaAltura) && $toleranciaAltura != 0)) {
                                            // Se especificaron ambas tolerancias
                                            $diferenciaTotal = sqrt(pow($diferenciaPeso, 2) + pow($diferenciaAltura, 2)); // Calcula la diferencia total usando la distancia euclidiana
                                        
                                            if ($diferenciaTotal < $diferenciaOptima) {
                                                $parejaOptima = $gallo2;
                                                $diferenciaOptima = $diferenciaTotal;
                                            }
                                        }
                                    ...
                                    if ($parejaOptima) {
                                        $parejas[] = [$gallo1, $parejaOptima];
                                         /* Excluir a los gallos emparejados de la lista de gallos disponibles
                                        $gallos = array_filter($gallos, function($gallo) use ($parejaOptima) {
                                        return $gallo['ID'] != $parejaOptima['ID'] && $gallo['familiasId'] != $parejaOptima['familiasId'];});*/
                                    } else {
                                        $gallosLibres[] = $gallo1;
                                    }
                                }
                            
                                // Limitar la cantidad de parejas al número máximo de peleas posibles
                                $cantidadPeleas = min(count($parejas), floor(count($gallos) / 2));
                                $parejas = array_slice($parejas, 0, $cantidadPeleas);
                            ...
                            <table class="table table-responsive-md table-sm ">
                                    ...
                                    <tbody>
                                        <?php foreach ($parejas as $pareja) :
                                            $gallo1 = $pareja[0];
                                            $parejaOptima = $pareja[1];

                                            $familiasId1 = $gallo1['familiasId'];
                                            $familiasId2 = $parejaOptima['familiasId'];
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td><?php echo $parejaOptima['anillo']; ?></td>
                                                <td><?php echo $familia2['nombre']; ?></td>
                                                <td><?php echo $parejaOptima['tamañoReal']; ?></td>
                                                <td><?php echo $parejaOptima['pesoReal']; ?></td>
